...
public class MainWindow2 extends JFrame implements TableModelListener {
		...
		for (Entrence e : entr) {
			...
			if (rekka == rekkaUI) {
				...
				if (n == JOptionPane.YES_OPTION) {
					...
					// TRY TO USE UPDATE
					Entrence tempToDelRow= fetchGlerskalinn.delete(rekkaUI);
					 
					myTableModel.refreshWhenDelete(tempToDelRow);
	...
	class MyTableModel extends AbstractTableModel {
		...
		public void refreshWhenDelete ( Entrence e){
			entlist.remove(e);
			//fireTableDataChanged();
			fireTableRowsDeleted(entlist.size() - 1, entlist.size() - 1);
		}
